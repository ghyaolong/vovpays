...
class ChooseAccountService
{
    ...
    public function getAccount(User $user, string $type, float $price)
    {
        ...
        // 淘宝红包金额整数
        if($this->pay_code == 'thb'){
            if(floor($price) != $price) return RespCode::PARAMETER_ERROR_PRICE;
        }

        // 根据挂号方式获取所有开启的账号:1商户后台挂号,2总后台挂号,3代理后台挂号,4三方挂号
        if ($add_account_type == 1) {
            if ($this->pay_code == 'thb' || $this->pay_code == 'alipay_receipt' || $this->pay_code == 'alipay_packets' || $this->pay_code == 'alipay' || $this->pay_code == 'wechat' || $this->pay_code == 'cloudpay') {
        ...
        //根据编码选择对应的账号
        if ($this->pay_code == 'thb' || $this->pay_code == 'alipay_receipt' || $this->pay_code == 'alipay_packets' || $this->pay_code == "alipay" || $this->pay_code == 'alipay_solidcode') {
