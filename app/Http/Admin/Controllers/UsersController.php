...
class UsersController extends Controller
{
    ...
    public function balance(Request $request)
    {
        ...
        if ($data['balance_type'] == 0)// 增加
        {
            $result = $this->statisticalService->updateUseridHandlingFeeBalanceIncrement($data['uid'], $data['amount']);
        }
        if ($data['balance_type'] == 1) // 减少
        {
            $result = $this->statisticalService->updateUseridHandlingFeeBalanceDecrement($data['uid'], $data['amount']);
        }
        if ($result) {
            return ajaxSuccess('修改成功');
        } else {
