...
class getOrderCallback extends Command
{
    ...
    public function orderCallback($json_str)
    {
        ...
        } else if ( strstr ($data['mark'], '收款') || strstr ($data['mark'], '固码') ) {
        ...
        if($order_type == 'recharge'){
            ...
            if (floatval($order->actual_amount) != floatval($data['money'])) return 4;
            ...
            $result = $rechargeService->update($order->id,['pay_status'=>1]);
            if (!$result) return 6;
            ...
            // 商户充值金额增加
            $statisticalService->updateUseridBalanceIncrement($user->id, $order->actual_amount);
            ...
            //商户后台挂号时：扣除预充值手续费
            if($add_account_type == 1 && $order->orderRate > 0){
                $statisticalService->updateUseridBalanceDecrement($user->id,$order->orderRate);
            }

            //代理后台挂号时：按照总后台收益扣除预充值手续费
            if($add_account_type == 3 && $order->orderRate > 0){
                $statisticalService->updateUseridBalanceDecrement($order->agent_id,$order->sysAmount);
            }
